...
if (( ${free_space} < 10 ));then
    ...
    /usr/bin/find /run/user/1007182/raddose3d/cache -mtime +2 -not -path "/run/user/1007182/raddose3d/cache" -exec rm -rf {} \;
elif (( ${free_space} < 25 ));then
    echo "Deleting less agressively hours"
    /usr/bin/find /run/user/1007182/raddose3d/cache -mmin +60 -not -path "/run/user/1007182/raddose3d/cache"  -exec rm -rf {} \;
elif (( ${free_space} < 50 ));then
    echo "Deleting more agressively tens of minutes"
    /usr/bin/find /run/user/1007182/raddose3d/cache -mmin +30 -not -path "/run/user/1007182/raddose3d/cache"  -exec rm -rf {} \;
elif (( ${free_space} < 75 ));then
    echo "Deleting more agressively 5 minutes"
    /usr/bin/find /run/user/1007182/raddose3d/cache -mmin +5 -not -path "/run/user/1007182/raddose3d/cache"  -exec rm -rf {} \;
elif (( ${free_space} < 90 ));then
    echo "Deleting more agressively one minute"
    /usr/bin/find /run/user/1007182/raddose3d/cache -mmin +1 -not -path "/run/user/1007182/raddose3d/cache"  -exec rm -rf {} \;
fi
/usr/bin/du -hsc /run/user/1007182/raddose3d/cache
echo "########"
