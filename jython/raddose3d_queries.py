...
class Raddose_API_client(object):
    ...
    def get_dose(
        ...
        input_parameters = {
            ...
            "xtalsize_x": xtalsize_x,
            "xtalsize_y": xtalsize_y,
            "xtalsize_z": xtalsize_z,
        ...
        return self._query_raddose3d_api(
            parameters=input_parameters, query_type="getdose"
        )

    def get_dose_with_assumptions(
        self,
        total_exposure_time,
        flux,
        energy_kev,
        ...
        input_parameters = {
            ...
            "beamsize_x": beamsize_x,
            "beamsize_y": beamsize_y,
            "xtalsize_x": beamsize_x,
            "xtalsize_y": beamsize_y,
            "xtalsize_z": beamsize_y,
            "oscillation_start": 0.0,
            "oscillation_end": 360.0,
            "energy_kev": energy_kev,
        }

        result = self.get_dose(**input_parameters)
        
        if result:
            required_dose = result[dose_method]
            self.logger.info("Calculated dose in MGy is:%s" % (required_dose))
            return required_dose
        else:
    ...
    def get_exposure(
        ...
        return self._query_raddose3d_api(
            parameters=input_parameters, query_type="getexposure"
        )

    def get_exposure_with_assumptions(
        self,
        total_dose,
        flux,
        energy_kev,
    ...
    def _query_raddose3d_api(self, parameters, query_type):

        query = {
            "getdose": self.raddose3d_api_url_dose,
            "getexposure": self.raddose3d_api_url_exposure,
        }

        try:
            self.conn = httplib.HTTPConnection(
                host=self.raddose3d_api_server_url, port=self.raddose3d_api_server_port
            )
